...
export default function ShopProductsManager() {
    ...
    // State mới cho Loại sản phẩm (Shop Categories)
    const [shopCategories, setShopCategories] = useState<ShopCategoryType[]>([]);
    const [selectedCategory, setSelectedCategory] = useState<string>('all'); // Lọc theo Loại sản phẩm
    ...
    useEffect(() => {
        // Đổi tên hàm để lấy cả Shop ID và Loại sản phẩm
        const fetchShopIdAndCategories = async () => {
            ...
            try {
                ...
                if (shopData && shopData.id) {
                    ...
                    // Ngay khi có shopId, tải danh mục của shop
                    try {
                        const categoriesData = await fetchShopCategories(); // GỌI API MỚI
    ...
    const loadProducts = useCallback(async () => {
        ...
        try {
            setLoading(true);
            setError('');
            // API cũ của bạn có `categoryId`, tôi đoán là `generic_id`
            // Chúng ta sẽ tải tất cả sản phẩm (categoryId = 0) và lọc ở client
            ...
            setProducts(data || []);
            // `setFilteredProducts` sẽ được gọi trong `useEffect` bên dưới
        } catch (error) {
    ...
    }, [shopId, sortBy]); // Bỏ `selectedCategory` vì chúng ta lọc ở client
    ...
    // Tách riêng useEffect để lọc
    useEffect(() => {
        filterProducts();
    }, [searchTerm, selectedCategory, products]); // Chạy khi 1 trong 3 thay đổi

    const filterProducts = () => {
        let filtered = [...products];

        // 1. Lọc theo "Loại sản phẩm" (shop_cate_id)
        if (selectedCategory !== 'all') {
            const cateId = parseInt(selectedCategory, 10);
            filtered = filtered.filter(p => p.shop_cate_id === cateId);
        }

        // 2. Lọc theo tên/mô tả
        if (searchTerm.trim()) {
    ...
    const handleAdd = () => {
        ...
        setCurrentProduct({
            ...
            image_url: '',
            status: 1,
            shop_cate_id: null, // Thêm trường này
        });
        setShowModal(true);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Đảm bảo gửi `shop_cate_id` (là null hoặc 1 số)
        const productDataToSend = {
            ...currentProduct,
            shop_cate_id: currentProduct.shop_cate_id || null
        };

        try {
            if (modalMode === 'add') {
                await createProduct(shopId, productDataToSend);
            } else if (modalMode === 'edit' && currentProduct.id) {
                await updateProduct(currentProduct.id, productDataToSend);
            }
    ...
    if (loading) { // Đơn giản hóa logic loading
        return (
            ...
            <div className="container mt-4">
                ...
                <div className="row g-3 mb-3">
                    ...
                    <div className="col-md-3">
                        {/* ===== START PHẦN THAY ĐỔI (LỌC LOẠI SẢN PHẨM) ===== */}
                        ...
                        <div className="modal-content" style={{borderRadius: '8px', border: 'none'}}>
                            ...
                            <form onSubmit={handleSubmit}>
                                <div className="modal-body" style={{padding: '24px'}}>
                                    ...
                                    {/* ===== START PHẦN THAY ĐỔI (DROPDOWN TRONG MODAL) ===== */}
                                    <div className="row">
                                        <div className="col-md-6 mb-3">
                                            <label className="form-label" style={{fontWeight: '600', fontSize: '13px', color: '#333', marginBottom: '8px'}}>
                                                Danh mục shop
                                            ...
                                            <select
                                                // Bỏ 'required' để cho phép giá trị null (chưa chọn)
                                                disabled={modalMode === 'view'}
                                                value={currentProduct.shop_cate_id || ''} // Dùng '' để khớp với <option value="">
                                                onChange={(e) => setCurrentProduct({...currentProduct, shop_cate_id: e.target.value ? Number(e.target.value) : null})}
                                                className="form-select"
                                                style={{borderRadius: '6px', padding: '10px 12px', fontSize: '14px', border: '1px solid #d9d9d9'}}
                                            >
                                        ...
                                        <div className="col-md-6 mb-3">
                                            {/* Đây là phần `category_id` (danh mục chính) mà bạn đã comment out */}
                                            {/* <label className="form-label" style={{fontWeight: '600', fontSize: '13px', color: '#333', marginBottom: '8px'}}>
                                                Danh mục chính
                                            </label>
                                            <select
