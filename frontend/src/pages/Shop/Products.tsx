...
// (PHIÊN BẢN SỬA LỖI HOÀN CHỈNH CUỐI CÙNG)

import React, { useState, useEffect, useCallback } from 'react'; // 1. Thêm 'React'
import { useNavigate, useSearchParams } from 'react-router-dom'; 
import { useAuth } from '../../context/AuthContext';

// 2. Sửa lỗi: Tách riêng 'type' và 'hàm'
...
// 3. Import 'Form' từ 'react-bootstrap'
...
export default function ShopProductsManager() {
  ...
  // (useEffect loadProducts giữ nguyên)
  useEffect(() => {
    if (shopId) {
        loadProducts();
    }
  ...
  // (handleAdd, handleEdit, handleView giữ nguyên)
  const handleAdd = () => {
  ...
  // (Hàm mới)
  const handleStatusToggle = async (productId: number, newStatus: number) => {
  ...
  // (Render loading, !shopId giữ nguyên)
  if (!shopId && !loading) {
    return (
  ...
  return (
    <div style={{backgroundColor: '#FAFAFA', minHeight: '100vh', paddingBottom: '40px'}}>
      ...
      <div className="container mt-4">
        ...
        {/* Products Table (NÂNG CẤP: Thay thế <td> Trạng thái) */}
            ...
            <tbody>
              {filteredProducts.map((product) => (
                <tr key={product.id} style={{borderBottom: '1px solid #f5f5f5'}}>
                  ...
                  {/* ===== SỬA LỖI: Cung cấp kiểu (type) cho `e` ===== */}
                  <td style={{padding: '16px 20px'}}>
                    <Form.Check 
                        type="switch"
                        id={`product-switch-${product.id}`}
                        checked={product.status === 1}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleStatusToggle(product.id, e.target.checked ? 1 : 0)}
                  ...
                  <td style={{padding: '16px 20px'}}>
                    <div className="d-flex gap-2 justify-content-end">
                      <button
                        ...
                        title="Xem chi tiết"
                        style={{ width: '32px', height: '32px', borderRadius: '6px', border: '1px solid #FFE0B2', backgroundColor: '#FFF8E1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                      >
                      ...
                      <button
                        ...
                        className="btn btn-sm p-0"
                        title="Xóa"
                        style={{ width: '32px', height: '32px', borderRadius: '6px', border: '1px solid #FFCDD2', backgroundColor: '#FFEBEE', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                      >
