...
export const getProductOnCategoryIdService = async (Category_id: number): Promise<Product[]> => {
    const [rows] = await pool.query("SELECT id, name, description, base_price, shop_id, image_url FROM products JOIN productimages on productimages.product_id = products.id where category_id = ?", [Category_id]);
